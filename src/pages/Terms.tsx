...
These Terms and Conditions (“Terms”, “T&C”) are between the entity you represent, or, if you do not indicate an entity in connection with the Services, you individually (“Customer”, “you” or “your”), and Zenfra, Inc. with its principal office at 541 Jefferson Ave. Suite 100, Redwood City CA 94063, United States of America (“Zenfra”, “the Company” “we”, “us”, or “our”). These terms incorporate by reference the Data Processing Agreement, the Privacy Policy, the Refund Policy and the Cookie Policy (together, they are the basis for your contractual relationship with Zenfra).

The Terms govern the use of:

    Services procured directly from Zenfra, except where a separate, mutually agreed master services agreement has been executed (such as Standard Contract for AWS Marketplace and any amendments, if Services are procured through AWS Marketplace);

    Services obtained through an authorized Zenfra partner, with the exception of provisions in Sections 5 and 6, which will not apply in such instances.
...
1.6. “Metrics” - means the measurements used for quantifying the Services usage with the following meaning:
    ...
    1.6.4. “Worker(s)” means a predefined set(s) of computing resources that are specifically optimized for the development and provisioning and deployment of cloud-based infrastructures based on IaC; a Worker can be either a self-hosted agent performing infrastructure management in a Customer-controlled environment (“Private Worker”) or any other software agent, provided and managed by Zenfra in a common secure worker pool (“Public Worker”).

1.7. “Services” means the Zenfra's specialized, continuous integration and deployment (CI/CD) platform for infra-as-code;

1.8. “Subscription” means enrollment for Services for a Subscription Plan on Subscription Details as defined in the Terms;

1.9. “Subscription Plan(s)” means Subscription offer(s) for use of the Services as described on https://zenfra.io/pricing/, which include paid plans (“Paid Subscription Plan(s)”) and the Free Plan;
...
3.3. License. Zenfra makes the Services available to you during the Subscription Period, subject to the provisions of the Terms and Subscription Details. Zenfra grants you a limited, non-exclusive, non-sublicensable, non-transferable right to access and use the Services and its Documentation during the Subscription Period, solely for your internal business purposes or your personal use.
...
4.9. Support Services. During the Subscription Period, we will provide support services depending on the Subscription Plan, as described in https://docs.zenfra.io/product/support/ incorporated to the Terms by reference.
...
5.2. Subscription Plans. The Services are available under the Subscription Plans presented on https://zenfra.io/pricing/ (and incorporated to the Terms by reference), describing in detail the relevant Metrics, Subscription Fees, Subscription Period, and the scope of the features included.
...
5.6.2. Downgrading the Subscription Plan. If you downgrade the Subscription Plan with the use of the self-service functionalities in your account, the changes to the Subscription Plan will take effect immediately, which might result in losing to the selected functionalities, such as add-ons. Your billing will be adjusted accordingly and you will be owed the prorated fees for the difference between your current Subscription Plan and the upgraded Plan for the remainder of the current billing cycle. To receive the refund, please contact ar@zenfra.io.
...
10.1. Exclusion of Liability In no event will Zenfra have any obligation or liability arising from (a) use or inability to use any Services if modified or combined with materials not provided by us; (b) statements or conduct of any third party on or in the Services, (c) any Customer Data, (d) any failure by Customer to comply with the Terms; and (e) damages suffered by Customer or Authorized Users, or any other person having arisen due to the third-party claims (other than described in Section 9.1), suspension or termination of the Services, or (f) any other reasons arising from Customer's fault or responsibility.

10.2. EXCLUSION OF DAMAGES. EXCEPT AS OTHERWISE PROVIDED IN SECTION 10.4 AND TO THE FULLEST EXTENT PERMITTED BY LAW, IN NO EVENT WILL ZENFRA OR ANY OF ITS LICENSORS OR SERVICE PROVIDERS BE LIABLE UNDER OR IN CONNECTION WITH THESE TERMS OR ITS SUBJECT MATTER UNDER ANY LEGAL OR EQUITABLE THEORY, INCLUDING BREACH OF CONTRACT, TORT (INCLUDING NEGLIGENCE), STRICT LIABILITY, AND OTHERWISE, FOR ANY: (a) LOSS OF PRODUCTION, USE, BUSINESS, REVENUE, OR PROFIT OR DIMINUTION IN VALUE; (b) IMPAIRMENT, INABILITY TO USE OR LOSS, INTERRUPTION, OR DELAY OF THE SERVICES; (c) LOSS, DAMAGE, CORRUPTION, OR RECOVERY OF DATA, OR BREACH OF DATA OR SYSTEM SECURITY; (d) COST OF REPLACEMENT GOODS OR SERVICES; (e) LOSS OF GOODWILL OR REPUTATION; OR (f) CONSEQUENTIAL, INCIDENTAL, INDIRECT, EXEMPLARY, SPECIAL, ENHANCED, OR PUNITIVE DAMAGES, REGARDLESS OF WHETHER SUCH PERSONS WERE ADVISED OF THE POSSIBILITY OF SUCH LOSSES OR DAMAGES OR SUCH LOSSES OR DAMAGES WERE OTHERWISE FORESEEABLE, AND NOTWITHSTANDING THE FAILURE OF ANY AGREED OR OTHER REMEDY OF ITS ESSENTIAL PURPOSE.

10.3. CAP ON MONETARY LIABILITY. ZENFRA WILL ONLY BE LIABLE FOR DIRECT DAMAGES EXCLUDING ANY SITUATION FOR WHICH WE ARE NOT RESPONSIBLE OR WHICH ARE CAUSED BY EVENTS OUTSIDE OUR REASONABLE CONTROL. HOWEVER, EXCEPT AS OTHERWISE PROVIDED IN SECTION 10.4, IN NO EVENT WILL THE COLLECTIVE AGGREGATE LIABILITY OF ZENFRA ARISING OUT OF OR RELATED TO THESE TERMS, WHETHER ARISING UNDER OR RELATED TO BREACH OF CONTRACT, TORT (INCLUDING NEGLIGENCE), STRICT LIABILITY, OR ANY OTHER LEGAL OR EQUITABLE THEORY, EXCEED THE GREATER OF (a) THE TOTAL AMOUNTS PAID TO ZENFRA UNDER THESE TERMS IN THE 6 MONTH PERIOD PRECEDING THE EVENT GIVING RISE TO THE CLAIM OR (b) THE AMOUNT OF 5000 USD. THE FOREGOING LIMITATIONS APPLY EVEN IF ANY REMEDY FAILS OF ITS ESSENTIAL PURPOSE.
...
11.3. Exercise of the Right to Withdraw. To exercise the right of withdrawal, you must inform us, Zenfra, Inc., of your decision to withdraw from the Terms by an unequivocal statement (e.g. an e-mail sent to legal@zenfra.io). To meet the withdrawal deadline, it is sufficient for you to send your communication concerning your exercise of the right of withdrawal before the withdrawal period expires.
...
12.3. Contact details. For any formal notices or complaints, please contact legal@zenfra.io. In any other matters, including any inquiry about the use of the Services, please contact us at contact@zenfra.io.
...
const Terms: React.FC = () => {
  ...
  // More sophisticated approach might involve markdown parsing if the format allows
      ...
      <main>
        <div className="max-w-4xl mx-auto px-4 py-8">
          {paragraphs.map((para, index) => {
            // Check for list items (simple check for lines starting with '    ')
            if (para.trim().startsWith('    ')) {
              // Further check if it's a sub-list item (e.g., 1.6.1)
              const isSubListItem = /^\s*\d+\.\d+\.\d+\./.test(para.trim());
              const listItems = para.split('\n').map((item, itemIndex) => (
                <li key={itemIndex} style={{ marginLeft: isSubListItem ? '4rem' : '2rem' }}>
                  {item.trim()}
                </li>
              ));
              return <ul key={index} style={{ marginBottom: '1rem', listStylePosition: 'inside' }}>{listItems}</ul>;
            }
            // Check for headings (simple check for '»' at the end)
            if (para.trim().endsWith('»')) {
               // Check for h2 (section headings)
               if (/^\d+\./.test(para.trim())) {
                   return <h2 key={index} className="text-2xl font-semibold mt-8 mb-4">{para.replace(/»/g, '').trim()}</h2>;
               }
               // Default to h3 if not a numbered section
               return <h3 key={index} className="text-xl font-semibold mt-6 mb-3">{para.replace(/»/g, '').trim()}</h3>;
            }
            // Check for table-like structure (simple check for lines with tabs)
            if (para.includes('\t')) {
                ...
                const dataRows = rows.slice(1).map((row, rowIndex) => (
                    <tr key={rowIndex}>
                        {row.split('\t').map((cell, cellIndex) => (
                            <td key={cellIndex} className="border border-gray-300 px-4 py-2">{cell.trim()}</td>
                        ))}
                    </tr>
                ));
                return (
                    <table key={index} className="table-auto border-collapse border border-gray-300 w-full mb-4">
                         <thead>
                           <tr>
                             {headers.map((header, headerIndex) => (
                               <th key={headerIndex} className="border border-gray-300 px-4 py-2 bg-gray-100 font-semibold text-left">{header}</th>
                             ))}
                           </tr>
                         </thead>
            ...
            // Handle the first paragraph (Last Updated date)
            if (index === 0) {
              return <p key={index} className="text-gray-600 mb-6">{para.trim()}</p>;
            }
            // Handle definition items (e.g., 1.1. "Authorized User")
            if (/^\d+\.\d+\.\s/.test(para.trim())) {
                 return <p key={index} className="mb-4"><strong className="font-semibold">{para.split(' ')[0]}</strong> {para.substring(para.indexOf(' ') + 1)}</p>;
            }
             // Handle sub-items (e.g., 2.1. Execution...)
            if (/^\d+\.\d+\.\s/.test(para.trim())) {
                 return <p key={index} className="mb-4"><strong className="font-semibold">{para.split(' ')[0]}</strong> {para.substring(para.indexOf(' ') + 1)}</p>;
            }
                ...
                {para.split('\n').map((line, lineIndex) => (
                  <React.Fragment key={lineIndex}>
                     ...
                     {line.includes('http://') || line.includes('https://') ? (
                       <a href={line.match(/https?:\/\/[^\s]+/)?.[0]} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                           {line}
                       </a>
                    ) : line}
