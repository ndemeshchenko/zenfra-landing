...
Thank you for choosing to be part of our community at Zenfra (“company”, “we”, “us”, or “our”). We are committed to protecting your personal information and your right to privacy. If you have any questions or concerns about our policy, or our practices with regards to your personal information, please contact us at privacy@zenfra.io.
...
Our servers are located in Ireland. If you are accessing our site from outside, please be aware that your information may be transferred to, stored, and processed by us in our facilities and by those third parties with whom we may share your personal information.
...
If you have questions or comments about this policy, you may email us at privacy@zenfra.io 


12. How can you review, update, or delete the data we collect from you?»

Based on the laws of some countries, you may have the right to request access to the personal information we collect from you, change that information, or delete it in some circumstances. To request to review, update, or delete your personal information, please email us at privacy@zenfra.io. We will respond to your request within 30 days.
`;
...
const Privacy: React.FC = () => {
  ...
  const paragraphs = privacyContent.split('\n\n');
          ...
          {paragraphs.map((para, index) => {
            ...
            if (para.trim().startsWith('    ')) {
              const listItems = para.split('\n').map((item, itemIndex) => (
                <li key={itemIndex} style={{ marginLeft: '2rem' }}>{item.trim()}</li>
              ));
            ...
            if (para.trim().endsWith('»')) {
                // Check for h1 (first heading)
                if (index === 0) {
                    return <h1 key={index} className="text-3xl font-bold mb-6">{para.replace(/»/g, '').trim()}</h1>;
                }
                // Check for h2 (section headings)
                if (/^\d+\./.test(para.trim())) {
                    return <h2 key={index} className="text-2xl font-semibold mt-8 mb-4">{para.replace(/»/g, '').trim()}</h2>;
                }
                // Check for h3 (sub-section headings)
                return <h3 key={index} className="text-xl font-semibold mt-6 mb-3">{para.replace(/»/g, '').trim()}</h3>;
            }
            // Check for account info section
            if (para.includes('Account Information')) {
                const parts = para.split('\n\n');
                return (
                    <div key={index}>
                        <h3 className="text-xl font-semibold mt-6 mb-3">{parts[0].replace(/»/g, '').trim()}</h3>
                        <p className="mb-4">{parts[1]}</p>
                        <p className="mb-4">{parts[2]}</p>
                    </div>
                );
            }
            return (
